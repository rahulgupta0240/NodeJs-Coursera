...
.options(cors.corsWithOptions, (req, res) => { res.sendStatus(200); })
    ...
    .get(cors.cors,(req, res, next) =>{
        Favorite.find({'postedBy': req.user_id})
    .populate('postedBy')
	.populate('dishes')
    .then((favorites) => {
        res.statusCode = 200;
    ...
    .post(cors.corsWithOptions,(req, res, next)=> {
        ...
        Favorite.find({'postedBy': req.user._id})
    ...
    delete(function (req, res, next) {
        Favorite.remove({'postedBy': req.user._id}, function (err, resp) {
            if (err) throw err;
            res.json(resp);
        })
    });
...
favoriteRouter.route('/:dishId')
    ...
    .delete(function (req, res, next) {
        ...
        Favorite.find({'postedBy': req.user._id}, function (err, favorites) {
            if (err) return err;
